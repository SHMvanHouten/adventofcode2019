...
class Day02Test {
    ...
    inner class Part1 {
        ...
        internal fun `1, 0, 0, 0 becomes 2, 0, 0, 0`() {
            ...
            assertThat(run(intCode), equalTo(listOf(2, 0, 0, 0, 99)))
        }

        @Test
        internal fun `1, 1, 1, 0 becomes 2, 1, 1, 0`() {
            val intCode = IntCode(1, 1, 1, 0, 99)
            assertThat(run(intCode), equalTo(listOf(2, 1, 1, 0, 99)))
        }
        ...
        internal fun `1, 0, 0, 1 becomes 1, 2, 0, 1`() {
            ...
            assertThat(run(intCode), equalTo(listOf(1, 2, 0, 1, 99)))
        }

        @Test
        internal fun `1, 2, 1, 1 becomes 1, 4, 1, 1`() {
            val intCode = IntCode(1, 2, 1, 1, 99)
            assertThat(run(intCode), equalTo(listOf(1, 3, 1, 1, 99)))
        }
        ...
        internal fun `1,0,0,0,99 becomes 2,0,0,0,99`() {
            ...
            assertThat(run(intCode), equalTo(listOf(2, 0, 0, 0, 99, 99)))
        }

        @Test
        internal fun `2,0,0,1 becomes 2, 4, 0, 1`() {
            val intCode = IntCode(2, 0, 0, 1, 99)
            assertThat(run(intCode), equalTo(listOf(2, 4, 0, 1, 99)))
        }
        ...
        internal fun `2,3,0,3,99 becomes 2,3,0,6,99`() {
            ...
            assertThat(run(intCode), equalTo(listOf(2, 4, 4, 5, 99, 9801)))
        }
        ...
        internal fun `99 stops the program`() {
            ...
            assertThat(run(intCode), equalTo(listOf(99, 0, 0, 0)))
        }
        ...
        internal fun `we run the next block when this block is finished`() {
            assertThat(
                run(IntCode(1, 0, 0, 1, 1, 0, 0, 1, 99)),
                equalTo(listOf(1, 2, 0, 1, 1, 0, 0, 1, 99))
            )
            assertThat(
                run(IntCode(1, 0, 0, 1, 1, 0, 0, 4, 99)),
        ...
        internal fun `1,1,1,4,99,5,6,0,99 becomes 30,1,1,4,2,5,6,0,99`() {
            ...
            assertThat(run(intCode), equalTo(listOf(30,1,1,4,2,5,6,0,99)))
        }

        @Test
        internal fun part1() {
            val input = parseInput(inputPt1)
            println(run(IntCode(input)))
            // 2890696
        }
    }
