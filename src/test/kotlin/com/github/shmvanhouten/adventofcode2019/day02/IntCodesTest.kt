...
class IntCodesTest {
    ...
    inner class Part1 {
        ...
        internal fun `1, 0, 0, 0 becomes 2, 0, 0, 0`() {
            ...
            computer.run()
            assertThat(computer.getIntCodes(), equalTo(listOf(2L, 0L, 0L, 0L, 99)))
        }
        ...
        internal fun `1, 1, 1, 0 becomes 2, 1, 1, 0`() {
            ...
            val computer = Computer(intCode = intCode)
            computer.run()
            assertThat(computer.getIntCodes(), equalTo(listOf(2L, 1L, 1L, 0L, 99)))
        }
        ...
        internal fun `1, 0, 0, 1 becomes 1, 2, 0, 1`() {
            ...
            val computer = Computer(intCode = intCode)
            computer.run()
            assertThat(computer.getIntCodes(), equalTo(listOf(1L, 2L, 0L, 1L, 99)))
        }
        ...
        internal fun `1, 2, 1, 1 becomes 1, 4, 1, 1`() {
            ...
            val computer = Computer(intCode = intCode)
            computer.run()
            assertThat(computer.getIntCodes(), equalTo(listOf(1L, 3L, 1L, 1L, 99)))
        }
        ...
        internal fun `1,0,0,0,99 becomes 2,0,0,0,99`() {
            ...
            val computer = Computer(intCode = intCode)
            computer.run()
            assertThat(computer.getIntCodes(), equalTo(listOf(2L, 0L, 0L, 0L, 99, 99)))
        }
        ...
        internal fun `2,0,0,1 becomes 2, 4, 0, 1`() {
            ...
            val computer = Computer(intCode = intCode)
            computer.run()
            assertThat(computer.getIntCodes(), equalTo(listOf(2L, 4L, 0L, 1L, 99)))
        }
        ...
        internal fun `2,3,0,3,99 becomes 2,3,0,6,99`() {
            ...
            val computer = Computer(intCode = intCode)
            computer.run()
            assertThat(computer.getIntCodes(), equalTo(listOf(2L, 4L, 4L, 5L, 99, 9801)))
        }
        ...
        internal fun `99 stops the program`() {
            ...
            val computer = Computer(intCode = intCode)
            computer.run()
            assertThat(computer.getIntCodes(), equalTo(listOf(99, 0L, 0L, 0L)))
        }

        @Test
        internal fun `we run the next block when this block is finished`() {
            val computer = Computer(intCode = IntCode(1L, 0L, 0L, 1L, 1L, 0L, 0L, 1L, 99))
            computer.run()
            assertThat(
                computer.getIntCodes(),
                equalTo(listOf(1L, 2L, 0L, 1L, 1L, 0L, 0L, 1L, 99))
            )
            val computer2 = Computer(intCode = IntCode(1L, 0L, 0L, 1L, 1L, 0L, 0L, 4L, 99))
            computer2.run()
            assertThat(
                computer2.getIntCodes(),
        ...
        internal fun `1,1,1,4,99,5,6,0,99 becomes 30,1,1,4,2,5,6,0,99`() {
            ...
            val computer = Computer(intCode = intCode)
            computer.run()
            assertThat(computer.getIntCodes(), equalTo(listOf(30L,1L,1L,4L,2L,5L,6L,0L,99)))
        }

        @Test
        internal fun part1() {
            val input = parseInput(inputPt1)
            val computer = Computer(intCode = IntCode(input))
            computer.run()
            assertThat(computer.getIntCodes()[0], equalTo(2890696L))
            // 2890696
        }
    }
