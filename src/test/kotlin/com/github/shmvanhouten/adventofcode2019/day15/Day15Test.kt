...
class Day15Test {
    ...
    inner class Part1 {
        ...
        inner class Droid {
            ...
            @Test
            internal fun `droid will accept input`() {
                val computer = FakeComputer()
                computer.output.offer(0) // to avoid nullpointer
            ...
            @Test
            internal fun `if droid can move north it will return 1`() {
                val computer = FakeComputer()
                computer.output.offer(1) // successful move output
        ...
        internal fun part1() {
            ...
            val (shortestPath, map) = pathFinder.findPathAndMap(droid)
            println(draw(map, shortestPath.last()))
            assertThat(shortestPath.size, equalTo(354))
        }
    }

    @Test
    internal fun part2() {
        assertThat(calculateTimeNeededToFillUp(IntCode(input)), equalTo(370))
    }
}

private fun draw(
    coordinates: Collection<Coordinate>,
    oxygenMachine: Coordinate
): String {
    return (minY(coordinates)!! - 1..maxY(coordinates) + 1).joinToString("\n") { y ->
        (minX(coordinates) - 1..maxX(coordinates) + 1).joinToString("") { x ->
            if (coordinates.contains(Coordinate(x, y))) {
                if(Coordinate(x,y) == oxygenMachine) {
                    "o"
                } else {
                    " "
                }
...
private fun maxX(coordinates: Collection<Coordinate>) =
    coordinates.map { it.x }.max()!!

private fun minX(coordinates: Collection<Coordinate>) =
    coordinates.map { it.x }.min()!!

private fun maxY(coordinates: Collection<Coordinate>) =
    coordinates.map { it.y }.max()!!

private fun minY(coordinates: Collection<Coordinate>) =
    coordinates.map { it.y }.min()
