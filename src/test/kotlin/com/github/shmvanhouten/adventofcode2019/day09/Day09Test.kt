package com.github.shmvanhouten.adventofcode2019.day09
...
class Day09Test {
    ...
    inner class Part1 {
        ...
        @Test
        internal fun `we support relative mode`() {
            val intCode = IntCode(109, 2, 2201, 0, 2, 0, 4, 0, 99)
            val computer = Computer(intCode)
            computer.run()
            assertThat(computer.output.first(), equalTo(2203L))
        }

        @Test
        internal fun `we support relative mode for parameter 3s`() {
            // 109, 2 add 2 to relativeBase
            // 20001, 1, 2, 2 intCodes[4 (2 + relativeBase)] = intCodes[1]  + intCodes[2] (2 + 20001)
            // 4, 4 write intCodes[4] to output
            val intCode = IntCode(109, 2, 20001, 1, 2, 2, 4, 4, 99)
            val computer = Computer(intCode)
            computer.run()
            assertThat(computer.output.first(), equalTo(20003L))
        }

        @Test
        internal fun `testInput 1 produces a copy of itself`() {
            val intCode = IntCode(109, 1, 204, -1, 1001, 100, 1, 100, 1008, 100, 16, 101, 1006, 101, 0, 99)
            val computer = Computer(intCode)
            computer.run()
            assertThat(
                computer.output,
        ...
        @Test
        internal fun `testInput 2 outputs a 16 digit number`() {
            val intCode = IntCode(1102, 34915192, 34915192, 7, 4, 7, 99, 0)
            val computer = Computer(intCode)
            computer.run()
            val output = computer.output.first()
            assertThat(output.toString().length, equalTo(16))
        }

        @Test
        internal fun `testInput 3 outputs the large number in the middle`() {
            // change EVERYTHING to longs -.-
            val intCode = IntCode(104L, 1125899906842624L, 99L)
            val computer = Computer(intCode)
            computer.run()
            assertThat(computer.output.first(), equalTo(1125899906842624L))
        }
