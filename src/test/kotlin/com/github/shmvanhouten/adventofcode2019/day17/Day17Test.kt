...
class Day17Test {
    ...
    inner class Part1 {
        ...
        internal fun `our printer prints the ascii string it receives`() {
            ...
            computer.output.offer('l'.toLong())
            computer.output.offer('l'.toLong())
            computer.output.offer('o'.toLong())

            assertThat(printer.print(), equalTo("Hello"))
        }

        @Test
        internal fun `the intersection is at 1,1`() {
            val map = """.#.
                        |###
                        |.#.
            """.trimMargin()
            assertThat(findIntersections(map), equalTo(listOf(Coordinate(1, 1))))
        }

        @Test
        internal fun `the intersection is at 2, 1`() {
            val map = """..#.
                        |####
                        |..#.
            """.trimMargin()
            assertThat(findIntersections(map), equalTo(listOf(Coordinate(2, 1))))
        }
        ...
        internal fun part1() {
            ...
            val intersectionsSum = intersections.sumBy { it.x * it.y }
            assertThat(intersectionsSum, equalTo(6244))
        }
    }
...
....#####......""".trimIndent()
            ...
            val startingPoint = findStartingPoint(input)
            val path = walkPath(scaffold, startingPoint)
            val routine = compressPathsToRoutines(path)

            assertThat(routine.main, equalTo("A,B,C,B,A,C\n"))
            assertThat(routine.a, equalTo("R,8,R,8\n"))
            assertThat(routine.b, equalTo("R,4,R,4,R,8\n"))
            assertThat(routine.c, equalTo("L,6,L,2\n"))
        }

        @Test
        internal fun `part 2`() {
            val alternativeInput = input.toMutableList()
            alternativeInput[0] = 2
            val scaffoldBot = ScaffoldBot(alternativeInput)
            assertThat(scaffoldBot.reportOnScaffold(), equalTo(1143523))
        }
